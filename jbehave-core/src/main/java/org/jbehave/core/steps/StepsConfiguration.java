 ...
 * {@link org.jbehave.core.steps.StepsFactory} and the in the
 * {@link org.jbehave.core.steps.Steps} implementations to customise its runtime
 * properties.
 * </p>
 * <p>
 * StepsConfiguration dependencies can be provided either via constructor or via
 * use* methods, which override the the default values of the dependency, which
 * is always set. The use methods allow to override the dependencies one by one
 * and play nicer with a Java hierarchical structure, in that does allow the use
...
public abstract class StepsConfiguration {
	...
	/**
	 * Map of starting words, keyed on step type, as derived from the keywords
	 * provided
	 */
	...
	/**
	 * Default no-op constructor, uses the default instances defined for member
	 * variables.
	 */
	...
	public StepsConfiguration useParameterConverters(
