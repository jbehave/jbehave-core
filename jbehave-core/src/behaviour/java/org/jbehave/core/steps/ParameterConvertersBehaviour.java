...
public class ParameterConvertersBehaviour {
	...
	public void shouldConvertValuesToNumbers() {
		...
		assertThat((Float) converters.convert("3.0", Float.class), equalTo(3.0f));
		assertThat((Float) converters.convert("3.0", float.class), equalTo(3.0f));
		assertThat((Long) converters.convert("3", Long.class), equalTo(3L));
		assertThat((Long) converters.convert("3", long.class), equalTo(3L));
		assertThat((Double) converters.convert("3.0", Double.class), equalTo(3.0d));
		assertThat((Double) converters.convert("3.0", double.class), equalTo(3.0d));
		assertThat((BigInteger) converters.convert("3", BigInteger.class), equalTo(new BigInteger("3")));
		assertThat((BigDecimal) converters.convert("3.0", BigDecimal.class), equalTo(new BigDecimal("3.0")));
	}

    @Test
    public void shouldConvertNaNAndInfinityValuesToNumbers() {
        ParameterConverters converters = new ParameterConverters();
        assertThat((Float) converters.convert("Infinity", Float.class), equalTo(Float.POSITIVE_INFINITY));
        assertThat((Float) converters.convert("-Infinity", Float.class), equalTo(Float.NEGATIVE_INFINITY));
        assertThat((Float) converters.convert("NaN", Float.class), equalTo(Float.NaN));
        assertThat((Double) converters.convert("NaN", Double.class), equalTo(Double.NaN));
        assertThat((Double) converters.convert("Infinity", Double.class), equalTo(Double.POSITIVE_INFINITY));
        assertThat((Double) converters.convert("-Infinity", Double.class), equalTo(Double.NEGATIVE_INFINITY));
    }
